...
(function () {
  ...
  if (galleryEl) {
    ...
    function closeLightbox() {
      ...
      // Xoá nội dung wrap → video đang chạy bị huỷ → dừng playback + giải phóng buffer.
      var wrap = document.getElementById('fb-lb-img-wrap');
    ...
    function lbGo(idx) {
      ...
      var wrap = document.getElementById('fb-lb-img-wrap');
      if (item && wrap) {
        // Rebuild phần tử mỗi lần đổi slide: chuyển giữa image/video/youtube
        // đều sạch — video/iframe cũ bị huỷ → tự ngừng phát.
        wrap.innerHTML = '';
        if (item.type === 'youtube') {
          var f = document.createElement('iframe');
          f.id = 'fb-lb-img';
          // `item.url` đã ở dạng /embed/<id> (do PHP build sẵn), chỉ append autoplay.
          f.src = item.url + (item.url.indexOf('?') === -1 ? '?' : '&') + 'autoplay=1';
          f.title = item.alt || 'YouTube';
